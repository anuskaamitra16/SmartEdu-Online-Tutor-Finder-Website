...
<html lang="en">
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css" integrity="sha512-PBwA0dkq2FuJT23YdtipTkLRGrWg4/JlFKZOiWOzUmNd43VuC5rsTbgzJaFtay/Lax1IYGxFWYPKZiCZCtc7Gg==" crossorigin="anonymous" referrerpolicy="no-referrer" />
...
<?php include("inc/notinav.html"); ?>
		<!-- End header -->
        <form action="jobpost.php" method="GET" class="search-form search-container">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                <div class="job-post">
                    ...
                    <?php
                        // Check if the teacher has already applied for this post
                        $post_id = $row['pid'];
                        $teacher_id = $_SESSION['tea']; // Assuming you have the teacher's ID in the session

                       $check_query = "SELECT * FROM studentrequests WHERE post_id = $post_id AND teacher_id = $teacher_id";
                        $check_result = $con->query($check_query);

                        if ($check_result->num_rows > 0) {
                            // If the teacher has already applied, display the "Applied" button
                            ?>
                            <button type="button" class="applied-button" disabled>Applied</button>
                            <?php
                        } else {
                            // If the teacher has not applied, display the "Apply" button
                            ?>
                             <a href="apply.php?sid=<?php echo $row['student_id']; ?>&pid=<?php echo $row['pid']; ?>" class="btn btn-primary">Apply</a>
<?php
                        }
                        ?>
                    </div>
            ...
            if(isset($_GET['subject']) && !empty($_GET['subject'])) {
                echo "<p>No job posts found matching the search criteria for '{$_GET['subject']}'.</p>";
            } else {
