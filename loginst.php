...
if(isset($_POST['loginst'])){
    $e=$_POST['email'];
    $p=MD5($_POST['pass']);

    $sel="SELECT * FROM students WHERE email='$e' AND password='$p'";
    $rs=$con->query($sel);
    
    if($rs->num_rows > 0){
        $row=$rs->fetch_assoc();
        $student_id = $row['sid']; // Assuming 'sid' is the column name in the 'students' table

        // Store student ID (sid) in session
        $_SESSION['stu'] = $student_id;

        // Query to check if the student has already posted
        $query = "SELECT * FROM studentpost WHERE student_id = $student_id";
        $result = $con->query($query);

        // If the student has already posted, redirect them to a different page
        if ($result->num_rows > 0) {
            header("location: showpost.php");
            exit();
        }
        // If the student hasn't posted before, allow them to access the create post page
        else {
            header("location: studentcreatepost.php");
            exit();
        }
    } else {
        // Invalid credentials
...
<html lang="en">
    ...
    <style>
        body {
            background-color: #ffdddd; /* Light beige background color */
        }
...
<div class="login-container">
        <h2 class="text-center mb-4">Student/Parent Login</h2>
        <form action="" method="post">
            ...
            <button type="submit" name="loginst" class="btn btn-primary btn-block">Login</button>
